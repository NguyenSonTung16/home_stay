...
export const MHTimKiemPhong = () => {
    ...
    const btn_henXemPhong = (phong: any) => {
        const daTonTai = danhSachQuanTam.some((p: any) => p.maphong === phong.maphong);
        if (!daTonTai) {
            const newList = [...danhSachQuanTam, phong];
            setDanhSachQuanTam(newList);
            localStorage.setItem("danhSachPhongQuanTam", JSON.stringify(newList));
            alert(`Đã thêm "${phong.tenphong}" vào danh sách quan tâm!`);
        } else {
    ...
    const btn_xoaQuanTam = (maPhong: number) => {
        const newList = danhSachQuanTam.filter((p: any) => p.maphong !== maPhong);
        setDanhSachQuanTam(newList);
        localStorage.setItem("danhSachPhongQuanTam", JSON.stringify(newList));
    };
