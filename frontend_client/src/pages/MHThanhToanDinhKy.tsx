...
export const MHThanhToanDinhKy: React.FC = () => {
  ...
  const fetchActiveInfoAndBills = async () => {
    try {
      ...
      if (infoRes.status === 401) { localStorage.removeItem('currentUser'); localStorage.removeItem('token'); setErrorMsg('Phi�n dang nh?p d� h?t h?n. Vui l�ng dang nh?p l?i.'); setLoading(false); return; }
